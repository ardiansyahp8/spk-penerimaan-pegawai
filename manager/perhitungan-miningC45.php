...
//#3# Jika atribut = total, maka hitung jumlah kasus total, jumlah kasus layak dan jumlah kasus tdk layak
            ...
            $sqlJumlahKasusTotal = mysql_query("SELECT COUNT(*) as jumlah_total FROM data_pegawai WHERE status is not null $kondisiAtribut");
            ...
            $sqlJumlahKasusLayak = mysql_query("SELECT COUNT(*) as jumlah_layak FROM data_pegawai WHERE status = 'Layak' AND status is not null $kondisiAtribut");
            ...
            $sqlJumlahKasusTidakLayak = mysql_query("SELECT COUNT(*) as jumlah_tidak_layak FROM data_pegawai WHERE status = 'Tidak Layak' AND status is not null $kondisiAtribut");
...
//#5# Jika atribut != total (atribut lainnya), maka hitung jumlah kasus total, jumlah kasus layak dan jumlah kasus tdk layak masing2 atribut
            ...
            while($rowNilaiAtribut = mysql_fetch_array($sqlNilaiAtribut)) {
                ...
                $sqlJumlahKasusTotalAtribut = mysql_query("SELECT COUNT(*) as jumlah_total FROM data_pegawai WHERE $kondisi");
                ...
                // hitung jumlah kasus layak
                $sqlJumlahKasusLayakAtribut = mysql_query("SELECT COUNT(*) as jumlah_layak FROM data_pegawai WHERE $kondisi AND status = 'Layak'");
                ...
                // hitung jumlah kasus TDK layak
                $sqlJumlahKasusTidakLayakAtribut = mysql_query("SELECT COUNT(*) as jumlah_tidak_layak FROM data_pegawai WHERE $kondisi AND status = 'Tidak Layak'");
...
//#7# Lakukan perhitungan entropy
                // perhitungan entropy
                $sqlEntropy = mysql_query("SELECT id, jml_kasus_total, jml_layak, jml_tdk_layak FROM mining_c45");
                while($rowEntropy = mysql_fetch_array($sqlEntropy)) {
                    $getJumlahKasusTotalEntropy = $rowEntropy['jml_kasus_total'];
                    $getJumlahKasusLayakEntropy = $rowEntropy['jml_layak'];
                    $getJumlahKasusTidakLayakEntropy = $rowEntropy['jml_tdk_layak'];
...
function insertAtributPohonKeputusan($atribut, $nilai_atribut)
{
    ...
    if ($rowInfGainMaxTemp['gain_ratio'] > 0) {
        ...
        while($rowInfGainMax = mysql_fetch_array($sqlInfGainMax)) {
            if ($rowInfGainMax['jml_layak'] == 0 AND $rowInfGainMax['jml_tdk_layak'] == 0) {
                $keputusan = 'Kosong'; // jika jml_layak = 0 dan jml_tdk_layak = 0, maka keputusan Null
            } else if ($rowInfGainMax['jml_layak'] !== 0 AND $rowInfGainMax['jml_tdk_layak'] == 0) {
                $keputusan = 'Layak'; // jika jml_layak != 0 dan jml_tdk_layak = 0, maka keputusan Layak
            } else if ($rowInfGainMax['jml_layak'] == 0 AND $rowInfGainMax['jml_tdk_layak'] !== 0) {
                $keputusan = 'Tidak Layak'; // jika jml_layak = 0 dan jml_tdk_layak != 0, maka keputusan Tidak Layak
            } else {
                $keputusan = '?'; // jika jml_layak != 0 dan jml_tdk_layak != 0, maka keputusan ?
            }
...
//#18# Jika atribut yang diinput kosong (atribut awal) maka insert ke pohon keputusan id_parent = 0
                ...
                mysql_query("INSERT INTO pohon_keputusan_c45 VALUES ('', '$rowInfGainMax[atribut]', '$rowInfGainMax[nilai_atribut]', 0, '$rowInfGainMax[jml_layak]', '$rowInfGainMax[jml_tdk_layak]', '$keputusan', 'Belum', '$kondisiAtribut', 'Belum')");
            }
...
//#19# Jika atribut yang diinput tidak kosong maka insert ke pohon keputusan dimana id_parent diambil dari tabel pohon keputusan sebelumnya (where atribut = atribut yang diinput)
                ...
                $sqlIdParent = mysql_query("SELECT id, atribut, nilai_atribut, jml_layak, jml_tdk_layak FROM pohon_keputusan_c45 WHERE atribut = '$atribut' AND nilai_atribut = '$nilai_atribut' order by id DESC LIMIT 1");
                while($rowIdParent = mysql_fetch_array($sqlIdParent)) {
                    // insert ke tabel pohon keputusan
                    mysql_query("INSERT INTO pohon_keputusan_c45 VALUES ('', '$rowInfGainMax[atribut]', '$rowInfGainMax[nilai_atribut]', $rowIdParent[id], '$rowInfGainMax[jml_layak]', '$rowInfGainMax[jml_tdk_layak]', '$keputusan', 'Belum', '', 'Belum')");
                    ...
                    $perhitunganParentPrePruning = loopingPerhitunganPrePruning($rowIdParent['jml_layak'], $rowIdParent['jml_tdk_layak']);
                    $perhitunganChildPrePruning = loopingPerhitunganPrePruning($rowInfGainMax['jml_layak'], $rowInfGainMax['jml_tdk_layak']);

                    // hitung average Pessimistic error rate child
                    $perhitunganPessimisticChild = (($rowInfGainMax['jml_layak'] + $rowInfGainMax['jml_tdk_layak']) / ($rowIdParent['jml_layak'] + $rowIdParent['jml_tdk_layak'])) * $perhitunganChildPrePruning;
                    ...
                    if ($perhitunganPessimisticChildIncrement > $perhitunganParentPrePruning) {
                        ...
                        if ($rowIdParent['jml_layak'] > $rowIdParent['jml_tdk_layak']) {
                            $keputusanPrePruning = 'Layak';
                        // jika jml tdk kasus layak lbh besar, maka keputusan == tdk layak
                        } else if ($rowIdParent['jml_layak'] < $rowIdParent['jml_tdk_layak']) {
                            $keputusanPrePruning = 'Tidak Layak';
                        }
...
function insertIterasi()
{
    ...
    if ($rowInfGainMaxIterasi['gain_ratio'] > 0) {
        ...
        while($rowInsertIterasiC45 = mysql_fetch_array($sqlInsertIterasiC45)) {
            ...
            mysql_query("INSERT INTO iterasi_c45 VALUES ('', $iterasiKe, '$kondisiAtribut', '$rowInsertIterasiC45[atribut]', '$rowInsertIterasiC45[nilai_atribut]', '$rowInsertIterasiC45[jml_kasus_total]', '$rowInsertIterasiC45[jml_layak]', '$rowInsertIterasiC45[jml_tdk_layak]', '$rowInsertIterasiC45[entropy]', '$rowInsertIterasiC45[inf_gain]', '$rowInsertIterasiC45[split_info]', '$rowInsertIterasiC45[gain_ratio]')");
            $iterasiKe++;
        }
    }
}
...
function replaceNull()
{
    ...
    while($rowReplaceNull = mysql_fetch_array($sqlReplaceNull)) {
        $sqlReplaceNullIdParent = mysql_query("SELECT jml_layak, jml_tdk_layak, keputusan FROM pohon_keputusan_c45 WHERE id = $rowReplaceNull[id_parent]");
        $rowReplaceNullIdParent = mysql_fetch_array($sqlReplaceNullIdParent);
        if ($rowReplaceNullIdParent['jml_layak'] > $rowReplaceNullIdParent['jml_tdk_layak']) {
            $keputusanNull = 'Layak'; // jika jml_layak != 0 dan jml_tdk_layak = 0, maka keputusan Layak
        } else if ($rowReplaceNullIdParent['jml_layak'] < $rowReplaceNullIdParent['jml_tdk_layak']) {
            $keputusanNull = 'Tidak Layak'; // jika jml_layak = 0 dan jml_tdk_layak != 0, maka keputusan Tidak Layak
        }
